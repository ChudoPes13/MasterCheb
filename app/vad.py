...
class VadSegmenter:
    ...
    def partial_pcm16(self, minimum_duration: float = 0.7) -> bytes | None:
        """Return the active utterance without changing endpointing state.

        It is deliberately a copy: Whisper can produce a provisional STT
        result while later PCM chunks keep accumulating for the final version.
        """
        if not self._in_speech:
            return None
        raw = bytes(self._buffer)
        minimum_bytes = int(self.sample_rate * 2 * minimum_duration)
        return raw if len(raw) >= minimum_bytes else None

    def flush(self) -> bytes | None:
        """Explicitly finish the current utterance when the client presses Stop."""
        if not self._in_speech:
            self._pre_roll.clear()
            return None
        return self._finalize()
