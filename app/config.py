...
class Settings:
    app_name: str = os.getenv("APP_NAME", "МастерЧеб — Маша")
    company_id: str = os.getenv("COMPANY_ID", "mastercheb")
    timezone: str = os.getenv("APP_TIMEZONE", "Europe/Moscow")

    host: str = os.getenv("APP_HOST", "127.0.0.1")
    port: int = _int("APP_PORT", 8001)
    ...
    tts_preload_on_startup: bool = _bool("TTS_PRELOAD_ON_STARTUP", True)
    ...
    def company_config_path(self) -> Path:
        return self.config_dir / "company.json"
    ...
    @property
    def tts_pronunciations_path(self) -> Path:
        return self.config_dir / "tts_pronunciations.json"

    @property
    def rag_dir(self) -> Path:
        return self.docs_dir / "rag"
