...
export function Chat({lang,t,online,voiceReady}:{lang:Lang;t:Copy;online:boolean|null;voiceReady:boolean}) {
 const [connected,setConnected]=useState(false), [consent,setConsent]=useState(false),[messages,setMessages]=useState<Message[]>([]),[input,setInput]=useState(""),[sound,setSound]=useState(false),[mode,setMode]=useState("text"),[recording,setRecording]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""),[lead,setLead]=useState<Record<string,string>>({}),[stage,setStage]=useState<string|null>(null),[submitted,setSubmitted]=useState(false),[speaking,setSpeaking]=useState(false);
 const ws=useRef<WebSocket|null>(null), scroller=useRef<HTMLDivElement>(null), stream=useRef<MediaStream|null>(null), ctx=useRef<AudioContext|null>(null), node=useRef<AudioWorkletNode|null>(null), recordingRef=useRef(false), audio=useRef<HTMLAudioElement|null>(null), queue=useRef<Blob[]>([]), soundRef=useRef(false), playEpoch=useRef(0), closing=useRef(false), started=useRef(0), timer=useRef<number|undefined>(undefined);
 function stopPlayback(){playEpoch.current++;queue.current=[];if(audio.current){audio.current.pause();URL.revokeObjectURL(audio.current.src);audio.current=null;}window.speechSynthesis?.cancel();setSpeaking(false)}
 function playNext(){if(audio.current||!queue.current.length||!soundRef.current)return;const b=queue.current.shift()!; const a=new Audio(URL.createObjectURL(b));audio.current=a;setSpeaking(true);a.onended=()=>{URL.revokeObjectURL(a.src);audio.current=null;setSpeaking(false);playNext()};a.onerror=()=>{audio.current=null;setSpeaking(false);setError(t.voiceError);playNext()};a.play().catch(()=>{URL.revokeObjectURL(a.src);audio.current=null;setSpeaking(false);setError(t.voiceError)})}
 function stopMic(send=true){if(!recordingRef.current)return;recordingRef.current=false;setRecording(false);window.clearTimeout(timer.current);node.current?.disconnect();node.current=null;stream.current?.getTracks().forEach(tr=>tr.stop());stream.current=null;void ctx.current?.close();ctx.current=null;if(send&&ws.current?.readyState===1){ws.current.send(JSON.stringify({event:"finish_voice"}));setBusy(true)}}
 ...
 function connect(){
  ...
  socket.onopen=()=>socket.send(JSON.stringify({event:"hello",language:lang,consent:"2026-09-05",session_id:sessionStorage.getItem("mastercheb-session-"+lang)}));
  socket.onmessage=e=>{
   if(e.data instanceof Blob){queue.current.push(e.data);playNext();return}
   ...
   if(p.event==="assistant_response"){setBusy(false);setMessages(m=>[...m,{role:"assistant",text:p.text}]);setLead(p.lead);setStage(p.stage);setSubmitted(p.submitted);
    if(lang!=="ru"&&soundRef.current&&"speechSynthesis" in window){const voices=speechSynthesis.getVoices();const voice=voices.find(v=>v.lang.startsWith(lang==="zh"?"zh":"en"));if(voice){const u=new SpeechSynthesisUtterance(p.text);u.voice=voice;u.lang=lang==="zh"?"zh-CN":"en-US";u.onstart=()=>setSpeaking(true);u.onend=()=>setSpeaking(false);u.onerror=()=>{setSpeaking(false);setError(t.voiceError)};speechSynthesis.speak(u)}else setError(t.localVoice)}
   }
   if(p.event==="stt_final"){stopMic(false);if(p.text){setMessages(m=>[...m,{role:"user",text:p.text}]);setBusy(true)}else setBusy(false)}
   if(p.event==="error"||p.event==="tts_error"){setBusy(false);setError(p.code==="voice_unavailable"||p.event==="tts_error"?t.voiceError:t.error);stopMic(false)}
  };
 ...
 async function startMic(){
  ...
  try{
   ...
   let buffer:number[]=[];let heard=false;let lastVoice=performance.now();started.current=performance.now();recordingRef.current=true;setRecording(true);setBusy(false);
 ...
 return <div className="chat-card" id="chat">
  ...
  <div className="chat-messages" ref={scroller} role="log" aria-live="polite">{messages.map((m,i)=><div key={i} className={"message "+m.role}>{m.role==="assistant"&&<span className="message-label">{t.masha}</span>}<p>{m.text}</p></div>)}{busy&&<div className="message assistant pending">{t.thinking}</div>}</div>
